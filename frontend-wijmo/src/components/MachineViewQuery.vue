    ...
    <v-card outlined style="border-radius: 0; max-height:15vh; margin-top:-10px;">
        ...
        <v-card-text>
            <v-row no-gutters style="margin: 15px 0px -30px -15px;">
                ...
                <v-col style="max-width:140px;">
                    <String class="attributes-list" label="ModelName" v-model="value.parameters.modelName" :editMode="editMode"/>
                </v-col>
                <v-col>
...
<script>
    export default {
        name: 'MachineViewQuery',
        components:{},
        props: {},
        data: () => ({
            editMode: true,
            value: {
                apiPath: 'findByNameOrModelName',
                parameters: {}
            },
        }),
        created() {
            this.value.parameters.name = '';
            this.value.parameters.modelName = '';
        },
        watch: {
        },
        methods: {
            search() {
                let search = null;
                if(this.value.parameters.name == ''  && this.value.parameters.modelName == '' ) {
                    search = null;
                }else{
                    search = this.value;
                }
